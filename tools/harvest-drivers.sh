...
main() {
	...
	# --- Check dependencies ---
	need_cmds=(losetup mount umount cp find xargs lspci du)
	...
	# Prune unused firmware files if firmware directory exists
	if [[ -d "${OUT}/lib/firmware" ]]; then
		prune_unused_firmware "${OUT}/lib/firmware"
	fi

	log_info "Harvest complete. Output at: ${OUT}"
}

# Only run main if called directly (not when sourced)
