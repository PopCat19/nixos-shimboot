...
if [[ -t 1 && -z "${NO_COLOR:-}" ]]; then
  ...
  MAGENTA=$'\e[35m'; CYAN=$'\e[36m'; BOLD=$'\e[1m'; DIM=$'\e[2m'; RESET=$'\e[0m'
else
...
prompt_yes_no() {
  ...
  while true; do
    if [[ "${default}" == "y" ]]; then
      read -r -p "$(echo -e "${BOLD}${prompt}${RESET} [${GREEN}Y${RESET}/n]: ")" ans || { echo "no"; return 0; }
      ans="${ans:-Y}"
    else
      read -r -p "$(echo -e "${BOLD}${prompt}${RESET} [y/${RED}N${RESET}]: ")" ans || { echo "no"; return 0; }
      ans="${ans:-N}"
    fi
    case "${ans}" in
...
collect_system_pknames() {
  ...
  if src="$(findmnt -no SOURCE / 2>/dev/null)"; then
    pk="$(pk_of "$src")"; add_system_pk "$pk"
  fi

  # Home
  if src="$(findmnt -no SOURCE /home 2>/dev/null || true)"; then
    [[ -n "$src" ]] && { pk="$(pk_of "$src")"; add_system_pk "$pk"; }
  fi
  ...
  if [[ -r /proc/swaps ]]; then
    ...
    while read -r filename type _; do
      ...
      if [[ "$type" == "partition" || "$filename" == /dev/* ]]; then
        pk="$(pk_of "$filename")"; add_system_pk "$pk"
      fi
