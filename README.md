...
> **This repository and its codebase is mostly generated by a Large-Language Model (such as GPT-5, GLM-4.6) directed by a lazy, non-programmer simpleton who happened to have time and interest to try these sort of things**.
> 
...
# NixOS Shimboot
I've started this project because no one (from what I've seen) publically created a NixOS bootable shimboot. Hence, this repo emerged where the existing scripts from [ading2210/shimboot](https://github.com/ading2210/shimboot) is mostly incompatible to build with a non-FHS distro like NixOS.
...
**TLDR: gnu/linux on (most) chromebooks, except it runs from a persistent USB and can run cool stuff like Arch ***btw*** (and most distros)**

## Why vibecode?
My controversial excuses for vibecoding this project are, but not limited to:
1. lazy (to learn fundamentals)
2. Reading dense docs for three hours make me kaput (except ArchWiki)
3. Wants NixOS and hyprland setup to run on a potato chromebook whilst having access to it (uniformity)
4. ???

## Why flake?
Sure, [nixos-shimboot-legacy](https://github.com/PopCat19/nixos-shimboot-legacy) barely worked to build a bootable NixOS with frankenstein scripts running on hopes and dreams, yet it wasn't functional enough to even get past LightDM. 

The user environment was borked, the system couldn't find anything upon logging into LightDM. (session PATH was also missing core commands: mkdir, systemctl, Hyprland weren't found in the user session, so login fails after LightDM) I really didn't understand why, as if I hadn't realized that we were shoving many, many impure tweaks to get it booting. (scripts are a pain to build NixOS entirely)
...
This project, in fact, already has a Cachix binary cache for the patched systemd and built nixos-shimboot closures. Here's some info about it:
