...
let
  ...
  resolveInitramfs = enableLUKS: if enableLUKS then patchedInitramfsLuks else patchedInitramfsBase;
...
in
{
  packages.${system} = {
    ...
    "shimboot-image-${board}-luks" = mkShimbootImage {
      headless = false;
      enableLUKS = true;
    };
    "shimboot-image-${board}-headless-luks" = mkShimbootImage {
      headless = true;
      enableLUKS = true;
    };
  };
